...
st.title("Medical Perplexity - Friendly Medical Search Assistant")
...
if st.button("Ask Medical Assistant"):
    if not (google_api_key and cx and gemini_api_key):
        st.error("API keys or CX missing! Please set in environment or config.")
    else:
        st.info(friendly_problem_understanding(user_input))
        ...
        if not results:
            ...
            with st.spinner("Summarizing..."):
                summary = summarize_with_gemini(results, user_input, gemini_api_key, gemini_model)
            st.header("Synthesized Medical Summary")
            st.success(summary)
            st.markdown("#### Top Sources & Web Snippets")
            for i, res in enumerate(results, 1):
                st.markdown(f"{i}. [{res['title']}]({res['url']})")
                st.write(res["text"])
                st.markdown("---")

st.markdown("""
_Results: Google Custom Search API.  
Summary: Gemini AI.  
Friendly chat: Custom user dialog module._
""")
