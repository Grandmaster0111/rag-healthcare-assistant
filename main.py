...
def answer_question(query):
    ...
    # Step 2: Rerank candidates
    reranked = rerank_cross_encoder(query, results)
    # Step 3: Build Gemini/HF prompt
    prompt = build_gemini_prompt(query, reranked, max_contexts=3)
    # Step 4: Generate answer (update call_gemini if using HF)
    answer = call_gemini(prompt, stream=False)
    truncated = get_truncated_gemini_answer(answer, max_words=40)
    ...
    # Optionally return everything in a tuple/dict:
    return {"answer": truncated, "sources": sources}
